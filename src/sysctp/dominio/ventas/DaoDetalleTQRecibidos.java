 ...
 * @author jarteaga
 */
public class DaoDetalleTQRecibidos 
{
    
    ConexionBD nueva=new ConexionBD();

    public void guardar(DetalleTiquetesRecibidos detalleTR)
    {
        try
        {
           ...
           ResultSet obj1=nueva.executeQuery("SELECT MAX(conse_num_consecutivo_tiqute)AS maximo,MIN(conse_num_consecutivo_tiqute)AS minimo FROM inventario.consecutivos");
            ...
            if(detalleTR.getSerial_inicial() >= minimo && detalleTR.getSerial_final()<= maximo )
            {
             ...
             if(obj.next()==true)
             {
                 //if(obj2.next()==true)
                // {
                      nueva.executeUpdateQuery("INSERT INTO ventas.detalle_recibidos(id_especificacion_tiquete,consecutivo_inicial,consecutivo_final,id_actacambio)"
